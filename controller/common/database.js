...
module.exports = class DataBase {
  ...
  async getBy(field, value, columns) {
    if (columns) {
      ...
      const result = await this.query(
        `SELECT ${ColumnNames.map((key) => `${key}`).join(", ")}  FROM ${
          this.tableName
        } WHERE ${field} = $1`,
        [value]
      );
  ...
  async DB_query(query, value) {
    console.log(query, value);
    return (await this.query(query, value)).rows;
  }
  ...
  multipleInsert(fields, data) {
    const query = format(
      `INSERT INTO ${this.tableName} (${fields.join(", ")}) VALUES %L`,
      data
    );
  ...
  delete(field, value) {
    return this.query(`DELETE FROM ${this.tableName} WHERE ${field} = $1`, [
      value,
    ]);
  }
};
